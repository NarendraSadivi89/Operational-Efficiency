...
def handle_snow(sql_agent, prompt, keywords):
    with st.spinner("Thinking..."):
        response = sql_agent.run(
            f"""You have access to glide_{os.getenv('snow_user')}.db and you are an sql expert who can query or join the tables based on the '{prompt}'. Run the select query needed in the background and give me the records or result set returned by query but don't give me the select query.
